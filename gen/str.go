...
func HasPrefix[S ~string](args ...S) Predicate[S] {
	return func(str S) bool {
		...
		for _, arg := range l {
			if string(str[:len(arg)]) == string(arg) { return true }
		}
		return false
	}
}
...
func HasSuffix[S ~string](args ...S) Predicate[S] {
	return func(str S) bool {
		l := Lim[S](len(str))(args)
		for _, arg := range l {
			if string(str[len(str)-len(arg):]) == string(arg) { return true }
		}
		return false
	}
}
...
func ReplacePrefix[S ~string](pats ...S) Monadic[S, S] {
	return func(str S) S {
		if len(pats)%2 != 0 { return str }
		for i := 0; i < len(pats); i += 2 {
			p := pats[i]
			if len(p) > len(str) { continue }

			// blumefmt incorrectly inlines this
			if string(p) == string(str[:len(p)]) { return S(string(pats[i+1]) + string(str[len(p):])) }
		}
...
func ReplaceSuffix[S ~string](pats ...S) Monadic[S, S] {
	return func(str S) S {
		if len(pats)%2 != 0 { return str }
		for i := 0; i < len(pats); i += 2 {
			if len(pats[i]) > len(str) { continue }
			p := pats[i]
...
func Replace[S ~string](pats ...S) Monadic[S, S] {
	return func(str S) S {
		if len(pats)%2 != 0 { return str }
		for i := 0; i < len(pats); i += 2 {
...
func ReplaceRegex[S ~string](pat string, rep string) Monadic[S, S] {
...
func Shift[S ~string](count int) Monadic[S, S] {
...
func Pop[S ~string](count int) Monadic[S, S] {
	return func(a S) (res S) {
		if len(a) < count { return }
		return S(string(a[:count]))
	}
