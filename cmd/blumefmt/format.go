...
func (iv *inlineVisitor) visit(node ast.Node) ast.Visitor {
	switch n := node.(type) {
	case *ast.IfStmt:
		// We only handle "if cond { singleStmt }" with no else (or else is nil).
	...
	case *ast.CommClause:
		// Same pattern for `select { case ch <- x: ... }`,
		// if you wanted to inline single statements there as well.
		// (Not shown in user examples, but you could do it similarly.)
		if len(n.Body) == 1 {
...
func basic(input []byte) (res []byte, err error) {
	...
	offset := 1
	matcher_if_fn := MatchRegex(`.*(if|func).*{.*}$`)
	// matcher_case := MatchRegex(`.*(default:|case).*$`)
	for i, line := range Split(str, true, "\n") {
		switch {
