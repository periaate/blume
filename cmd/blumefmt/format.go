...
// 	switch iv.out {
// 	case true: return iv.visitOut(node)
// 	default: return iv.visit(node)
// 	}
// }
//
...
// 	stmtEnd := stmt.End()
//
// 	if !open.IsValid() || !close.IsValid() || !stmtStart.IsValid() || !stmtEnd.IsValid() { return }
//
...
// 	if len(cc.Body) != 1 { return token.NoPos, token.NoPos }
...
// 	if len(comm.Body) != 1 { return token.NoPos, token.NoPos }
// 	openPos := comm.Colon
...
// 			if regs[j].openBraceOffset < regs[i].openBraceOffset { regs[i], regs[j] = regs[j], regs[i] }
// 		}
// 	}
//
...
// 		if ch == '{' && !wrong { out.WriteString(" ") }
// 		if iv.out && wrong { out.WriteString("\n") }
// 	}
// 	// Finally, write the remainder of the file.
// 	if cursor < len(src) { out.Write(src[cursor:]) }
//
// 	return out.Bytes()
// }
//
...
// 	for i, line := range str.Split(str, true, "\n") {
// 		switch {
