...
import (
	...
	"github.com/periaate/blume/yap/based"
)
...
func KinoFormat(tim time.Time) (res string) {
	...
	if includeY {
		res += based.Encode(year - 1970)
	}
	if includeM {
		res += based.Encode(int(month) - 1)
	}
	if includeD {
		res += based.Encode(day - 1)
	}
	if includeh {
		res += based.Encode(hour)
	}
	if includem {
		res += based.Encode(min)
	}
	if includes {
		res += based.Encode(sec)
	}
	return
}
